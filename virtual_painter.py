...
with mp_hands.Hands(min_detection_confidence=0.85, min_tracking_confidence=0.5, max_num_hands=1) as hands:
    while cap.isOpened():
        ...
        # to improve performance, optinally mark the image
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if len(points) != 0:
                    ...
                    non_sel = [0, 3, 4] # indexes of the fingers taht need to be done in the selection mode
                    if (fingers[1] and fingers[2]) and all(fingers[i] == 0 for i in non_sel):
                        xp, yp = [x1, y1]

                        # selescting the color or eraser
                        if y1 < 125:
                            if 170 < x1 < 295:
                                header = overlay_list[0]
                                draw_color = (0, 0, 255)
                    ...
                    if all(fingers[i] == j for i, j in zip(range(0,5), selecting)) or all(fingers[i] == j for i, j in zip(range(0,5), selecting)):
        ...
        # setting the header in video
        image[0:125, 0:width] = header

        # image preproccessing to produce image
